...
Очередной сборник новостей Qt, на этот раз за последние полгода с [прошлой статьи](/post/354972). Релизы 5.11 и 5.12, реинкарнация PySide, внезапные похороны Qbs, обновление инструментов дизайна GUI и другое.

![ДДПВ](https://habrastorage.org/webt/yi/jl/mn/yijlmn6hjzvihibrburqqunvcjc.jpeg)

Интересной КПДВ я не придумал, потому вместо неё просто ДДПВ - это к нам летом в офис приходил фотограф на фотосессию под разные нужды, из которой я и подрезал фотку коллеги.
<cut />

Сначала о нарушенных обещаниях. В предыдущей статье в конце были голосования за публикацию перевода [поста из официального блога](http://blog.qt.io/blog/2018/05/03/qt-microncontrollers-mcu/) про портирование Qt на микроконтроллеры и за написание обзора [Safe Renderer](https://doc.qt.io/QtSafeRenderer/qtsr-overview.html), и оба голосования показали, что такие статьи были бы интересны. Но в публикации микроконтроллерной статьи НЛО автору отказало: "*Публикации рекламного характера вне корпоративного блога и хаба «Я пиарюсь» запрещены правилами сайта*". Сложно сказать, что именно там было рекламного (*можете посмотреть английский оригинал и оценить*), но в таком случае про Safe Renderer тогда не было смысла даже и пытаться (*потому что это исключительно коммерческая фича*). Так что простите, если кто ждал.
...
- [Ещё про MCU](#mcu)
- [Отказ от Qbs](#qbs)
- [Изменения в лицензировании](#license)
...
[Улучшена производительность](http://blog.qt.io/blog/2018/11/16/qt-quick-performance-improvements-qt-5-12-lts/). В основном это касается движка QML и JavaScript. Кстати, он теперь [соответствует стандарту ECMAScript 7](https://bugreports.qt.io/browse/QTBUG-47735).

Релиз [Qt Remote Object](https://doc.qt.io/qt-5.11/qtremoteobjects-gettingstarted.html) - механизма межпроцессорного взаимодействия как на одном хосте, так и между разными хостами по сети. До этого модуль был в статусе превью.

Релиз [Qt Quick WebGL](http://blog.qt.io/blog/2018/11/23/qt-quick-webgl-release-512/), он же Qt WebGL стриминг - то есть возможность стримить GUI приложения, работающего на удалённом хосте (*устройстве без дисплея*), и отображать его в браузере на десктопе или планшете:

![Qt Quick WebGL](https://habrastorage.org/webt/sp/zg/-l/spzg-lohcrc74m4xtfhjjcbrdh8.jpeg)

Очень прикольная штука, я прямо восторгом с ней поигрался, но откровенно говоря я не представляю, кто и зачем будет ей пользоваться. Серьёзно, если я и так собираюсь работать с девайсом из браузера, то зачем я буду городить GUI на Qt Quick и стримить его в WebGL? Не проще ли тогда просто запустить на девайсе нормальный веб-сервер, а GUI на клиенте будет HTML/CSS/JS без вот этого всего?
...
Pointer Handlers вышли из статуса превью и переименовались в [Input Handlers](https://doc.qt.io/qt-5/qtquickhandlers-index.html). Это модуль для обработки ввода с клавиатуры, мыши, стилуса или тачскрина. В связи с этим в какой-то момент следует ожидать "устаревания" [MouseArea](https://doc.qt.io/qt-5/qml-qtquick-mousearea.html).
...
В Qt for Device Creation появились так называемые [Qt Board Support Packages](https://doc.qt.io/QtForDeviceCreation/qtdc-qbsp.html). Это те же самые Yocto образ и тулчейн, только теперь в виде отдельно загружаемого и подключаемого к установщику пакета. [Смысл QBSP](http://blog.qt.io/blog/2018/12/14/board-support-package-commercial-build-enablers-qt-partners/) заключается в поддержке партнёров-вендоров железа, чтобы им было удобнее создавать Boot to Qt образы для своих девайсов.
...
* [релиз](http://blog.qt.io/blog/2018/10/25/qt-design-studio-1-0-released/).
...
И вот [Qt Design Studio](https://doc.qt.io/qtdesignstudio/index.html) - это тот же самый Qt Quick Designer, только с дополнительным функционалом. И хотя это теперь самостоятельное приложение, но по факту это покалеченная копия Qt Creator, которая просто запускается сразу в режиме дизайна Qt Quick, хотя и с возможностью работы с QML.

Из нового функционала: линия времени для работы с анимациями, удобные диалоги для настройки этих анимаций, компонент live-preview для предпросмотра изменений на лету (*как в отдельном окне, так и на присоединённом планшете или другом устройстве*), и другой функционал. Примечательно, что все эти вещи вряд ли когда-нибудь попадут обратно в Qt Quick Designer.

Qt Design Studio предназначена для дизайнеров, и подразумевается, что они будут создавать в ней дизайн приложения, передавать результат - `.ui.qml` файлы - разработчикам, а разработчики уже продолжат работать с ними в полноценном Qt Creator'е:
...
Продолжается развитие [Qt 3D Studio](https://doc.qt.io/qt3dstudio/qt3dstudio-index.html).

За это время вышли версии [2.0](http://blog.qt.io/blog/2018/06/26/qt-3d-studio-2-0-released/) ([более подробный обзор](http://blog.qt.io/blog/2018/06/27/whats-qt-3d-studio-scene/)), [2.1](http://blog.qt.io/blog/2018/10/01/qt-3d-studio-2-1-released/) и [2.2](http://blog.qt.io/blog/2018/12/13/qt-3d-studio-2-2-released/).
...
Также был обозначен план объединить Qt 3D Studio и Qt Design Studio в единый инструмент, то есть вместо двух это будет одно приложение для работы с 2D и 3D.

## <anchor>kuesa</anchor>Анонс Kuesa

Говоря о 3D, тут KDAB выпустили свой инструмент для работы с 3D - [Kuesa](https://www.kdab.com/kdab-releases-kuesa-for-3d-asset-creation-and-integration-workflow/).

В отличие от Qt 3D Studio, они не стали тратить ресурсы на собственный инструмент для 3D моделирования, а позволяют дизайнерам работать с их привычными инструментами (*3DS Max, Blender*), и далее разработчик может использовать экспортированные [glTF](https://en.wikipedia.org/wiki/GlTF) модели уже в Qt. Для удобства в наличии также есть приложение для предпросмотра модели и наименований компонентов, чтобы разработчик знал как к ним обращаться у себя в коде, не открывая модель в оригинальном 3D редакторе.

На мой взгляд Kuesa является конкурирующим Qt 3D Studio решением (*и судя по всему более удачным*), и это досадно, потому что вместо того чтобы параллельно заниматься одним и тем же эти усилия можно было бы потратить на что-то другое (*я сейчас не обязательно про KDAB*).
...
Списка отличий от [PyQt](https://riverbankcomputing.com/software/pyqt/intro) нет, хотя и разработчики заверяют, что их немного и число их сокращается. С точки зрения коммерческого лицензирования, с PySide у вас "всё включено" в стоимость лицензии Qt без дополнительно платы, а с PyQt надо ещё платить в Riverbank. С точки зрения Open Source, PySide доступен как под GPLv3, так и под LGPLv3, в то время как PyQt доступен только под GPLv3.

## <anchor>mcu</anchor>Ещё про MCU
...
Пост собрал две сотни комментариев (*рекорд для нашего блога*), и собрал бы больше, но комментарии к постам автоматически закрываются через две недели. Вот [тут](http://www.opennet.ru/opennews/art.shtml?num=49519) ещё есть комментарии на русском.

То есть, несколько лет сообществу твердили о том как Qbs прекрасен, какой это шаг вперёд, не говоря уже о заверениях что Qbs будет официальной системой сборки в Qt 6 и всем надо на неё переходить (*и народ таки начал переходить*), и тут его вдруг закрывают таким стремительным домкратом.
...
Немного улучшились условия коммерческих лицензий для стартапов: теперь доступны компоненты и из Device Creation тоже (*готовые образы на основе Yocto Linux, тулчейны для кросс-компиляции и т.д.*), однако для распространения продуктов-устройств рантаймы покупать всё равно потребуется (*логика такая, что если есть деньги на железо, то должны найтись и на рантаймы*).

Лицензия для стартапов выдаётся бесплатно на год и потом может быть продлена ещё на год. Юридически это полноценная коммерческая лицензия без необходимости выполнять требования GPL/LGPL. После первого года (*или двух*) она превращается в обычную лицензию за полную стоимость. Но пока не превратилась, действуют следующие ограничения:

* годовой доход компании или разработчика должен быть меньше 100 000 долларов, иначе вы не квалифицируетесь как стартап;
...
В этом году [Qt World Summit](https://www.qt.io/qtws18/home/) прошёл два раза: Бостоне и потом в Берлине. Записей выступлений пока нет, есть только короткий [видео-обзор](https://youtu.be/qSGVv0HFcIM) берлинского, а также [пост от KDAB](https://www.kdab.com/kdab-demos-at-qt-world-summit-berlin/).

Грядёт обновление иконок приложений:

![Новые иконки Qt](https://habrastorage.org/webt/yt/8s/vz/yt8svzf77lve-oeiqaqaealnvmc.png)
...
На этом с новостями всё. Я кстати даже завёл отдельный документ, в который записываю что происходит из значимого, чтобы когда придёт время очередной статьи не вспоминать в муках что же там было такого за последние полгода/год. Надеюсь, такая система не подвела и я ничего не пропустил.
